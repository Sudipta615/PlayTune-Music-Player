...
pub struct AudioEngine {
    ...
    visualizer_tap: Arc<crate::analysis::FftVisualizerTap>,
    ...
    /// Wall-clock time of the first failure in the current burst of
    /// recovery attempts. Used to enforce a cooldown: if recovery has
    /// been failing for more than `RECOVERY_COOLDOWN_SECS`, the attempt
    /// counter is reset so the engine can retry instead of being
    /// permanently stuck at the MAX_RECOVERY_ATTEMPTS ceiling.
    /// `None` when no recovery has been attempted yet or when the
    /// previous burst was successful (reset on success).
    stream_recovery_burst_start: Option<std::time::Instant>,
...
impl AudioEngine {
    ...
    pub fn new(config: EngineConfig) -> Result<Self, EngineError> {
        ...
        Ok(Self {
            ...
            audio_output: None,
            pipeline,
            visualizer_tap: Arc::new(crate::analysis::FftVisualizerTap::new(512, 65)),
            stream: None,
            config,
            position_secs: 0.0,
            duration_secs: 0.0,
            ...
            stream_recovery_burst_start: None,
    ...
    pub fn start(&mut self) -> Result<(), EngineError> {
        ...
        self.pipeline.update_sample_rate(self.output_sample_rate as f32);
        ...
        if let Err(e) =
            std::thread::Builder::new().name("tc-device-monitor".into()).spawn(move || {
                use cpal::traits::{DeviceTrait, HostTrait};
    ...
    pub fn send_command(&self, cmd: EngineCommand) {
        ...
        match self.cmd_tx.send_timeout(cmd, timeout) {
            ...
            Err(crossbeam::channel::SendTimeoutError::Timeout(cmd)) => {
                warn!(
                    "Engine command channel full; dropping command after {:?} timeout: {:?}",
                    timeout, cmd
                );
            }
    ...
    pub fn set_volume(&mut self, vol: f32) {
        let clamped = if vol.is_finite() { vol.clamp(0.0, 1.0) } else { 0.0 };
    ...
    pub fn load_track(&mut self, path: &std::path::Path) -> Result<DecodeInfo, EngineError> {
        ...
        let loudness_meta = crate::decode::symphonia_decoder::extract_loudness_metadata(path);
        self.pipeline.apply_loudness_metadata_outgoing(Some(loudness_meta));
    ...
    pub fn tick(&mut self) {
        ...
        if state == PlaybackState::Playing {
            ...
            if self.audio_output.is_none() {
                ...
                if count > 0 {
                    self.visualizer_tap.process_samples(&dummy_buf[..count], 2);
                }
            }
    ...
    pub fn visualizer_tap(&self) -> Arc<crate::analysis::FftVisualizerTap> {
        Arc::clone(&self.visualizer_tap)
    }
    ...
    pub fn set_config(&mut self, config: EngineConfig) {
        ...
        p.set_loudness_mode(match config.loudness.mode {
            config::LoudnessMode::Off => crate::dsp::loudness::LoudnessMode::Off,
            ...
            config::LoudnessMode::EbuR128 => crate::dsp::loudness::LoudnessMode::EbuR128,
        });
            ...
            p.set_eq_band(
                ...
                crate::dsp::equalizer::EqBandParams {
                    ...
                    filter_type: match band.filter_type {
                        ...
                        config::FilterType::LowShelf => {
                            crate::dsp::equalizer::EqFilterType::LowShelf
                        }
                        config::FilterType::HighShelf => {
            ...
            || config.crossfade.duration_ms != self.config.crossfade.duration_ms
        {
            ...
            p.mixer_mut().set_duration_ms(
                config.crossfade.duration_ms as u64,
                self.output_sample_rate as f32,
            );
        }
        if config.performance_mode != self.config.performance_mode {
            // Snapshot the volume gain and current playback state before
            // rebuilding the pipeline. The rebuild creates fresh DSP nodes
            // (GainProcessor, FadeProcessor, etc.) which reset their internal
            // state — without this snapshot, the volume would snap to 1.0
            // and the seek-fade would reset, causing an audible discontinuity
            // during playback.
            let saved_volume = self.pipeline.volume.current_gain();
            let saved_volume_target = self.pipeline.volume.target_gain;
            let saved_speed = self.speed;

            self.pipeline = DspPipeline::from_config(&config, self.output_sample_rate as f32);

            // Restore the snapshotted state so playback continues smoothly.
            self.pipeline.set_volume(saved_volume_target);
            self.pipeline.volume.gain = saved_volume;
            self.pipeline.set_speed(saved_speed);
        }
        ...
        if backend_changed {
            ...
            if let Err(e) = self.recover_output_stream() {
                error!("Failed to recover stream after backend/device change: {}", e);
            }
        }
    }

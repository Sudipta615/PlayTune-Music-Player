...
impl ConvolutionEngine {
    ...
    pub fn load_ir_from_samples(
        ...
        if ir_samples.is_empty() {
            return Err(ConvolutionError::FileLoad("IR contains no samples".to_string()));
        }
        let len = ir_samples.len().min(self.max_ir_length);

        // Determine if the IR is stereo (different L/R) or mono.
        let stride = (len / 512).max(1);
        let is_stereo = ir_samples[..len].iter().step_by(stride).any(|(l, r)| (l - r).abs() > 1e-5);
    ...
    pub fn process(&mut self, left: f32, right: f32) -> (f32, f32) {
        ...
        if self.overlap_available > 0 {
            ...
            // Apply wet/dry mix with exponential smoothing (L-DSP-1)
            ...
            let mix = self.current_wet_mix;
            let mixed_l = left * (1.0 - mix);
            let mixed_r = right * (1.0 - mix);
            (mixed_l, mixed_r)
        }
    }
    ...
    fn process_block(&mut self) {
        ...
        if left_ok && right_ok {
            // Copy IFFT results into pre-allocated scratch buffers first.
            let n = self
            ...
            let fft_size = self.fft_size;
            let overlap_left: &mut Vec<f32> = &mut self.overlap_left;
            let overlap_right: &mut Vec<f32> = &mut self.overlap_right;
            let overlap_read_pos: &mut usize = &mut self.overlap_read_pos;
            let overlap_available: &mut usize = &mut self.overlap_available;
            let dropped_frames: &mut u64 = &mut self.dropped_frames;
            let scratch_left: &[f32] = &self.scratch_left;
            let scratch_right: &[f32] = &self.scratch_right;
            Self::add_to_overlap_impl(
                &scratch_left[..n],
                &scratch_right[..n],
                overlap_left,
                overlap_right,
    ...
    fn add_to_overlap_impl(
        left: &[f32],
        right: &[f32],
        overlap_left: &mut [f32],
        overlap_right: &mut [f32],
        overlap_read_pos: &mut usize,
        overlap_available: &mut usize,
        dropped_frames: &mut u64,
        fft_size: usize,
    ) {
        let frames = left.len().min(fft_size);
        let write_start = *overlap_read_pos + *overlap_available;

        // This replaces the previous resize() which could allocate on the
        // real-time thread and caused unbounded memory growth.
        let buf_len = overlap_left.len();
        if write_start + frames > buf_len {
            // Shift unread data to the beginning of the buffer
            let shift_amount = *overlap_read_pos;
            if shift_amount > 0 && *overlap_available > 0 {
                // M-BM-1: use copy_within (memmove) instead of
                // element-by-element copy for ~2-4x faster compaction.
                overlap_left.copy_within(shift_amount..shift_amount + *overlap_available, 0);
                overlap_right.copy_within(shift_amount..shift_amount + *overlap_available, 0);
            }
            // Zero out the area after the shifted data to prevent stale
            // values from being added into during the overlap-add
            for i in *overlap_available..buf_len {
                overlap_left[i] = 0.0;
                overlap_right[i] = 0.0;
            }
            *overlap_read_pos = 0;
        ...
        let mut write_start = *overlap_read_pos + *overlap_available;
        if write_start + frames > buf_len {
            let overflow = (write_start + frames) - buf_len;
            // Advance the read position to discard the oldest samples
            let discard = overflow.min(*overlap_available);
            *overlap_read_pos += discard;
            *overlap_available -= discard;
            *dropped_frames += discard as u64;
            log::warn!(
            ...
            write_start = *overlap_read_pos + *overlap_available;
        }
        let writable = if write_start < buf_len { (buf_len - write_start).min(frames) } else { 0 };

        // Overlap-add: accumulate into existing buffer data
        for i in 0..writable {
            let idx = write_start + i;
            overlap_left[idx] += left[i];
            overlap_right[idx] += right[i];
        }

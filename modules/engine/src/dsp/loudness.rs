...
impl LoudnessNormalizer {
    ...
    pub fn set_track_metadata(&mut self, meta: &LoudnessMetadata) {
        ...
        let safe_rg_track_peak = meta.replaygain_track_peak.filter(|v| v.is_finite() && *v >= 0.0);
        let safe_rg_album_peak = meta.replaygain_album_peak.filter(|v| v.is_finite() && *v >= 0.0);
        ...
        let gain_db = match self.mode {
            ...
            LoudnessMode::TrackReplayGain => {
                safe_rg_track_db.map(|rg| rg + self.preamp_db).unwrap_or(0.0)
            }
            LoudnessMode::AlbumReplayGain => {
                safe_rg_album_db.map(|rg| rg + self.preamp_db).unwrap_or(0.0)
            }
        ...
        let adjusted_gain = if adjusted_gain.is_finite() { adjusted_gain } else { 0.0 };
...
mod tests {
    ...
    fn test_true_peak_guard() {
        ...
        assert!(guarded_gain <= unguarded_gain, "True peak guard should reduce gain when needed");
    }
}

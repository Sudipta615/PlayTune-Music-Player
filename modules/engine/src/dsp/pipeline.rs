...
impl DspPipeline {
    pub fn from_config(config: &EngineConfig, sample_rate: f32) -> Self {
        ...
        loudness_out
            .set_true_peak_guard(config.loudness.true_peak_guard, config.loudness.true_peak_dbtp);
            ...
            if let Some(ref ir_path) = config.convolution.ir_path {
                ...
                if path.exists() {
                    match convolution.load_ir_from_file(path) {
                        Ok(()) => log::info!("Loaded IR: {}", path.display()),
                        Err(e) => log::warn!("Failed to load IR {}: {}", path.display(), e),
                    }
                }
            }
        }
    ...
    pub fn apply_loudness_metadata_outgoing(&mut self, metadata: Option<LoudnessMetadata>) {
        self.out_loudness.set_track_metadata(&metadata.unwrap_or_default());
    }

    pub fn apply_loudness_metadata_incoming(&mut self, metadata: Option<LoudnessMetadata>) {
        self.in_loudness.set_track_metadata(&metadata.unwrap_or_default());
    }
    ...
    pub fn update_sample_rate(&mut self, sample_rate: f32) {
        ...
        self.out_preamp.set_slew_rate(1.0 / (PREAMP_RAMP_DURATION_MS * 0.001 * sample_rate));
        self.in_preamp.set_slew_rate(1.0 / (PREAMP_RAMP_DURATION_MS * 0.001 * sample_rate));
        self.volume.set_slew_rate(1.0 / (self.volume_fade_ms * 0.001 * sample_rate));
        self.seek_fade.set_sample_rate(sample_rate);
        // Convert the mixer's crossfade duration using the OLD sample rate.
        // Using the new rate made the ms->frames round trip a no-op, so the
        // crossfade length silently drifted on every output-rate change.
        let crossfade_ms = self.mixer.duration_ms(old_sample_rate.max(1.0));

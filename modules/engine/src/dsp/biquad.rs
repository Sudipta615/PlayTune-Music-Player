...
impl BiquadCoeffs {
    ...
    fn validate_params(sample_rate: f32, freq: f32, q: f32) -> (f32, f32, f32) {
        ...
        // Clamp Q to a safe minimum and maximum. P-22: a very
        // high Q (e.g. 1000+) at audio frequencies produces extremely narrow
        // peaks with coefficients exceeding f32 precision near DC/Nyquist.
        // The standard clamp range is [0.1, 24]; we use [0.01, 100] to be
        // more permissive but still bounded.
...
impl BiquadState {
    ...
    pub fn process(&mut self, sample: f32, coeffs: &BiquadCoeffs) -> f32 {
        debug_assert!(sample.is_finite(), "BiquadState::process called with non-finite sample");
...
impl SmoothedBiquad {
    ...
    pub fn process_frame(&mut self, frame: &mut AudioFrame) {
        if frame.num_channels <= 1 {
            // Mono: filter channel 0 and mirror the result to channel[1]
            // so both slots stay consistent (AudioFrame::mono() duplicates
            // the sample to both slots; mirroring the filtered result
            // preserves that invariant).
            let out = self.states[0].process(frame.channels[0], &self.current);
            frame.channels[0] = out;
            frame.channels[1] = out;
        } else {

...
impl StereoEnhancer {
    ...
    pub fn new() -> Self {
        Self { width: 1.0, current_width: 1.0, slew_rate: 0.001, enabled: false }
    }
    ...
    pub fn process(&mut self, left: f32, right: f32) -> (f32, f32) {
        ...
        // Dezippering: smoothly approach target width. Always run this so
        // current_width stays in sync with the latest target — even when
        // the target is 1.0 (passthrough), we need current_width to
        // converge to 1.0 so a future target change starts from the right
        // baseline.
...
mod tests {
    ...
    fn test_widening_increases_stereo_separation() {
        ...
        assert!(wide_diff > normal_diff, "Widening should increase stereo separation");
    }
    ...
    fn test_width_clamped() {
        ...
        assert!((enhancer.width() - 2.0).abs() < 1e-5, "Width should be clamped to 2.0");

        enhancer.set_width(-1.0); // Should be clamped to 0.0
        assert!((enhancer.width() - 0.0).abs() < 1e-5, "Width should be clamped to 0.0");
    }
    ...
    fn test_disabled_is_passthrough() {
        ...
        let (l, r) = enhancer.process(0.8, 0.2);
        assert!((l - 0.8).abs() < 1e-5, "Disabled enhancer should pass through");
        assert!((r - 0.2).abs() < 1e-5, "Disabled enhancer should pass through");
    }
    ...
    fn test_mid_preserved() {
        ...
        assert!((input_mid - output_mid).abs() < 1e-5, "Mid component should be preserved");
    }
}

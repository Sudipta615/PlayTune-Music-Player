...
impl EqBandParams {
    ...
    pub fn peaking(frequency: f32, gain_db: f32, q: f32) -> Self {
        Self { frequency, gain_db, q, filter_type: EqFilterType::Peaking, enabled: true }
    }

    /// Create a low-shelf band
    pub fn lowshelf(frequency: f32, gain_db: f32, q: f32) -> Self {
        Self { frequency, gain_db, q, filter_type: EqFilterType::LowShelf, enabled: true }
    }

    /// Create a high-shelf band
    pub fn highshelf(frequency: f32, gain_db: f32, q: f32) -> Self {
        Self { frequency, gain_db, q, filter_type: EqFilterType::HighShelf, enabled: true }
    }
}
...
impl ParametricEq {
    ...
    pub fn set_preamp_db(&mut self, gain_db: f32) {
        if !gain_db.is_finite() {
            log::warn!("ParametricEq::set_preamp_db: non-finite value {}; ignoring", gain_db);
            return;
        }
    ...
    pub fn set_post_gain_db(&mut self, gain_db: f32) {
        if !gain_db.is_finite() {
            log::warn!("ParametricEq::set_post_gain_db: non-finite value {}; ignoring", gain_db);
            return;
        }
        let clamped = gain_db.clamp(-30.0, 30.0);
    ...
    pub fn process(&mut self, left: f32, right: f32) -> (f32, f32) {
        ...
        let target_scale = if self.peak_envelope > headroom_linear && self.peak_envelope > 0.0 {
    ...
    pub fn set_bass_shelf(&mut self, gain_db: f32) {
        if !gain_db.is_finite() {
            log::warn!("ParametricEq::set_bass_shelf: non-finite value {}; ignoring", gain_db);
            return;
        }
        let clamped = gain_db.clamp(-30.0, 30.0);
    ...
    pub fn set_treble_shelf(&mut self, gain_db: f32) {
        if !gain_db.is_finite() {
            log::warn!("ParametricEq::set_treble_shelf: non-finite value {}; ignoring", gain_db);
            return;
        }
        let clamped = gain_db.clamp(-30.0, 30.0);

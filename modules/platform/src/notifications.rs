...
pub(crate) fn dispatch_notification_sync(title: &str, body: &str) -> Result<(), PlatformError> {
    #[cfg(target_os = "linux")]
    {
        ...
        let safe_title = escape_notify_markup(title);
        let safe_body = escape_notify_markup(body);
        let status = Command::new("notify-send").arg(&safe_title).arg(&safe_body).status();
        match status {
            Ok(s) if s.success() => Ok(()),
            Ok(s) => Err(PlatformError::Other(format!("notify-send exited with status: {}", s))),
            Err(e) => Err(PlatformError::Other(format!("Failed to run notify-send: {}", e))),
        }
    }
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if status.success() {
            ...
            Err(PlatformError::Other(format!("osascript exited with status: {}", status)))
        }
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let ps_script = "[Windows.UI.Notifications.ToastNotificationManager, Windows.UI.Notifications, ContentType = WindowsRuntime] | Out-Null; \
                         ...
                         [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('PlayTune').Show($toast)";
        ...
        match status {
            Ok(s) if s.success() => Ok(()),
            Ok(s) => {
                Err(PlatformError::Other(format!("PowerShell toast exited with status: {}", s)))
            }
            Err(e) => Err(PlatformError::Other(format!("Failed to run PowerShell: {}", e))),
        }
    }

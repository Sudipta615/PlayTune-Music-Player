...
pub struct PlatformIntegration {
    ...
    #[cfg(target_os = "linux")]
    mpris_notify_tx: Option<SyncSender<MprisPropertyChanged>>,
}
...
impl PlatformIntegration {
    ...
    pub fn new() -> Result<(Self, MediaKeyReceiver), PlatformError> {
        let (action_tx, action_rx) = mpsc::sync_channel(256);
    ...
    pub fn action_sender(&self) -> SyncSender<MediaKeyAction> {
    ...
    pub fn set_mpris_status(&mut self, status: MprisPlaybackStatus) {
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            if let Some(ref tx) = self.mpris_notify_tx {
                let _ = tx.try_send(MprisPropertyChanged::PlaybackStatus);
            }
        }
    }
    ...
    pub fn set_mpris_track(&mut self, info: MprisTrackInfo) {
        ...
        if let Some(ref mut ctrl) = self.media_controls {
            let duration = info
                .length_microseconds
                .filter(|&us| us > 0)
                .map(|us| std::time::Duration::from_micros(us.max(0) as u64));
            ctrl.set_metadata(
                info.title.as_deref(),
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            if let Some(ref tx) = self.mpris_notify_tx {
                let _ = tx.try_send(MprisPropertyChanged::TrackMetadata);
            }
        }
    }
    ...
    pub fn set_mpris_volume(&mut self, volume: f32) {
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            if let Some(ref tx) = self.mpris_notify_tx {
                let _ = tx.try_send(MprisPropertyChanged::Volume);
            }
        }
    }
    ...
    pub fn set_mpris_shuffle(&mut self, shuffle: bool) {
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            if let Some(ref tx) = self.mpris_notify_tx {
                let _ = tx.try_send(MprisPropertyChanged::Shuffle);
            }
        }
    }
    ...
    pub fn set_mpris_loop_status(&mut self, status: &str) {
        #[cfg(target_os = "linux")]
        {
            let validated = match status {
                "None" | "Track" | "Playlist" => status.to_string(),
                other => {
                    log::warn!("set_mpris_loop_status: invalid status '{}', ignoring", other);
                    return;
                }
            };
            if let Some(ref state) = self.mpris_state {
                let mut s = state.lock();
                s.loop_status = validated;
            }
            if let Some(ref tx) = self.mpris_notify_tx {
                let _ = tx.try_send(MprisPropertyChanged::LoopStatus);
            }
        }
        let _ = status;
    }
    ...
    pub fn is_media_controls_available(&self) -> bool {
        self.media_controls.as_ref().is_some_and(|c| c.is_available())
    }
    ...
    pub fn register_mpris(&mut self, identity: &str) -> Result<(), PlatformError> {
        ...
        let (notify_tx, notify_rx) = mpsc::sync_channel::<MprisPropertyChanged>(64);
    ...
    pub fn set_mpris_position(&mut self, position_microseconds: i64) {
        let clamped = position_microseconds.max(0);
        #[cfg(target_os = "linux")]
        {
            if let Some(ref state) = self.mpris_state {
                let mut s = state.lock();
                s.position_microseconds = clamped;
            }
        }

        // Update cross-platform media controls.
        if let Some(ref mut ctrl) = self.media_controls {
            ctrl.set_position(std::time::Duration::from_micros(clamped as u64));
        }
    }

    /// Update MPRIS playback rate.
    #[cfg(target_os = "linux")]
    pub fn set_mpris_rate(&mut self, rate: f32) {
        if let Some(ref state) = self.mpris_state {
            let mut s = state.lock();
            s.rate = rate;
        }
        if let Some(ref tx) = self.mpris_notify_tx {
            let _ = tx.try_send(MprisPropertyChanged::Rate);
        }
    }

    /// Non-Linux stub for set_mpris_rate (no MPRIS state to update).
    #[cfg(not(target_os = "linux"))]
    pub fn set_mpris_rate(&mut self, _rate: f32) {}

    /// Update the playback position AND emit an MPRIS Seeked signal.
    /// This method is the discontinuous-seek counterpart to
    /// `set_mpris_position`. On Linux it sends a `PositionSeeked` notification
    /// through `mpris_notify_tx`, which the D-Bus event loop treats as a
    /// trigger to set `pending_seeked = true` and emit the Seeked signal on
    /// the next iteration. On macOS/Windows, souvlaki handles position
    /// updates internally and there is no separate Seeked signal to emit.
    pub fn set_mpris_seek(&mut self, position_microseconds: i64) {
        let clamped = position_microseconds.max(0);
        #[cfg(target_os = "linux")]
        {
            if let Some(ref state) = self.mpris_state {
            ...
            if let Some(ref tx) = self.mpris_notify_tx {
                let _ = tx.try_send(MprisPropertyChanged::PositionSeeked);
            }
        }
        if let Some(ref mut ctrl) = self.media_controls {
            ctrl.set_position(std::time::Duration::from_micros(clamped as u64));
    ...
    /// Update MPRIS capability flags (CanGoNext, CanGoPrevious, CanPlay,
    /// CanPause, CanSeek).
    #[cfg(target_os = "linux")]
    pub fn set_mpris_capabilities(
        &mut self,
        can_go_next: bool,
        ...
        if let Some(ref state) = self.mpris_state {
            let mut s = state.lock();
            s.can_go_next = can_go_next;
            s.can_go_previous = can_go_previous;
            s.can_play = can_play;
            s.can_pause = can_pause;
            s.can_seek = can_seek;
        }
        // Note: we don't emit PropertiesChanged for capability changes
        // because the D-Bus event loop's notify channel doesn't have
        // variants for Can* properties. Clients will pick up the new
        // values on their next property query (most poll every few
        // seconds). Adding the variants is a future enhancement.
    }

    /// Non-Linux stub for set_mpris_capabilities (no MPRIS state to update).
    #[cfg(not(target_os = "linux"))]
    pub fn set_mpris_capabilities(
        &mut self,
        _can_go_next: bool,
        ...
        // No-op on non-Linux platforms — souvlaki handles capability
        // reporting internally and doesn't expose these flags.
    }
}
...
mod tests {
    ...
    fn test_mpris_track_update() {
        ...
        let info = MprisTrackInfo {
            ...
            track_id: Some("/org/playtune/track/1".to_string()),
            ..Default::default()
        };
    ...
    fn test_custom_shortcut() {
        let (mut platform, _rx) = PlatformIntegration::new().unwrap();
        platform.add_shortcut(KeyboardShortcut::new("P", MediaKeyAction::PlayPause).ctrl().alt());
    ...
    fn test_async_notification() {
        let (platform, _rx) = PlatformIntegration::new().unwrap();
        platform.send_notification("Test", "Async notification test");
    }
}

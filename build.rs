...
fn add_msvc_runtime_search_paths() {
    ...
    // First, use the environment that a VS developer shell normally sets.
    ...
    // Fallback: derive the VC toolchain root from cl.exe itself.
    if let Some(tool) = cc::windows_registry::find_tool(&target, "cl.exe") {
        if let Some(cl_path) = tool.path() {
            let mut root = cl_path.to_path_buf();
...
fn main() {
    ...
    if cfg!(target_os = "macos") {
        ...
        // Make the MSVC runtime library location explicit so link.exe can
        // always resolve msvcp140.lib even when the CI environment is missing
        // the default LIB search path entries.
        add_msvc_runtime_search_paths();

        // The C++ standard library import library on MSVC.
        println!("cargo:rustc-link-lib=dylib=msvcp140");
    } else {
        println!("cargo:rustc-link-lib=dylib=stdc++");
    }
